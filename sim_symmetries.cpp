...
{   
    ...
    unsigned n_symmetries = ss.is_symmetry_or_should_multiply_amplitude(y);
    bool symmetry_check_passed = n_symmetries != 0;
    if (symmetry_check_passed)
    {
    ...
    if (a.sign!=0) {
        double amp_increment = ((a.sign)*(1-2*overlap)*(1.0/double(one<<(num_nodes-a.pow2))));
        amp_increment *= n_symmetries;
