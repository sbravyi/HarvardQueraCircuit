...
use super::{augmented_gauss_jordan::AugmentedGaussJordan, matrix::BitMatrix};

pub struct BackwardsSubstitution {
    pub solution: BitVec,
    pub gj: AugmentedGaussJordan,
}
...
impl BackwardsSubstitution {
    pub fn zero(n: usize) -> Self {
        Self {
            ...
            gj: AugmentedGaussJordan::zero(n, n + 1)
        }
    }
    ...
    pub fn solve(&mut self, u: &BitMatrix, b: &BitVec) -> Option<()> {
        ...
        while i > 0 {
            i -= 1;
            let mut tmp = self.gj.rows[i][self.gj.augmented_col_n];
